...
def train(attn_implementation=None):
    ...
    if "mistral" in model_args.model_name_or_path.lower() or "mixtral" in model_args.model_name_or_path.lower() or "zephyr" in model_args.model_name_or_path.lower():
        ...
        rank0_print("Model path does not match a known tokenizer name; falling back to right-padded AutoTokenizer.")
        tokenizer = transformers.AutoTokenizer.from_pretrained(
            model_args.model_name_or_path,
            cache_dir=training_args.cache_dir,
            model_max_length=training_args.model_max_length,
        ...
        if training_args.gradient_checkpointing:
            ...
            if hasattr(model, "get_input_embeddings"):
                ...
                if emb is not None:
                    ...
                    hooked_embeddings.append(type(emb).__name__)
            ...
            if base_model is not None and hasattr(base_model, "get_input_embeddings"):
                ...
                if emb is not None:
                    emb.register_forward_hook(make_inputs_require_grad)
                    hooked_embeddings.append(type(emb).__name__)
            inner_model = getattr(getattr(model, "base_model", None), "model", None)
            if inner_model is not None and hasattr(inner_model, "get_input_embeddings"):
                emb = inner_model.get_input_embeddings()
                if emb is not None:
                    emb.register_forward_hook(make_inputs_require_grad)
                    hooked_embeddings.append(type(emb).__name__)
            rank0_print(f"Gradient checkpointing input hooks registered on: {hooked_embeddings}")
